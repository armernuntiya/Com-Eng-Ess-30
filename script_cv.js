...
const getCompEngEssCid = async () => {
  ...
  await fetch(
    ...
    .then((response) => response.json())
    .then((data) => {for (let j=0 ;j<data.data.length;j++){
     
      item1.push({"course":cv_cid[i],"item":data.data[j].itemid,"subject":cv_cid2[i]})
        } 
    }
    )
    .catch((error) => console.error(error));
    ...
    for (let i=0;i<item1.length;i++ ){
      await fetch(
        ...
        .then((data) => { 
            ...
            item2.push({"subject":item1[i].subject,"id":data.data.itemid,"title":data.data.title,"due":data.data.duetime,"timeMo":montha[d.getMonth()],"timeD":d.getDate(),"timeH":d.getHours(),"timeM":d.getMinutes(),"finished":0})
             }
            }
    ...
    console.log(item2)
    item2.sort((a,b)=> {return a.due-b.due;})
    item2.map((item) => {  
      if (item.timeM<10){
